...
class HeartMuLa(PreTrainedModel):
    ...
    def move_kv_caches(self, device: torch.device, dtype: torch.dtype = None) -> None:
        """Move all KV caches to the specified device.

        This is necessary when using mmgp lazy loading, as setup_caches()
        creates caches on CPU but forward pass runs on CUDA.
        """
        from .llama_blocks import ExpandableKVCache
        for module in self.modules():
            if hasattr(module, "kv_cache") and module.kv_cache is not None:
                if isinstance(module.kv_cache, ExpandableKVCache):
                    module.kv_cache.ensure_device(device, dtype)
