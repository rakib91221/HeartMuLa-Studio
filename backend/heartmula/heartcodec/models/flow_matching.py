...
# Monkey-patch torch.Tensor.masked_fill to fix device mismatch
# The issue is that mask is created from indices (which may be CPU) but all_codes
# comes from codebooks (which mmgp moves to CUDA). This causes masked_fill to fail.
_original_masked_fill = torch.Tensor.masked_fill

def _patched_masked_fill(self, mask, value):
    """Patched masked_fill that moves mask to same device as self."""
    if mask.device != self.device:
        mask = mask.to(self.device)
    return _original_masked_fill(self, mask, value)

# Apply the patch globally
torch.Tensor.masked_fill = _patched_masked_fill
...
class FlowMatching(nn.Module):
    ...
    def inference_codes(
        ...
        # Move entire VQ embed to same device to avoid mask device mismatch
        # This is needed because vector_quantize_pytorch may create masks on wrong device
        if vq_device != torch.device("cpu"):
            self.vq_embed = self.vq_embed.to(vq_device)
        ...
        # Ensure cond_feature_emb is on same device
        if self.cond_feature_emb.weight.device != quantized_feature_emb.device:
            self.cond_feature_emb = self.cond_feature_emb.to(quantized_feature_emb.device)
