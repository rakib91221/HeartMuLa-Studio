...
class HeartCodec(PreTrainedModel):
    ...
    def detokenize(
        ...
        restore = False
        restore_flow = False
        # Ensure vq_embed is on the same device as codes
        target_device = codes.device
        if self.flow_matching.vq_embed.layers[0]._codebook.embed.device != target_device:
            self.flow_matching.vq_embed.to(target_device)
            restore = True
        # Also ensure the flow_matching module (estimator, cond_feature_emb) is on the correct device
        if hasattr(self.flow_matching, 'estimator'):
            est_param = next(self.flow_matching.estimator.parameters(), None)
            if est_param is not None and est_param.device != target_device:
                self.flow_matching.to(target_device)
                restore_flow = True
